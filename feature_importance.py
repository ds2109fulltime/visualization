...
def show_feat_importance(importance,names,model_type, figsize=(10,8)):
    '''   
    This function shows a graph with feature importance values of a trained model
    ------------------
    
    Args:
    importance = here we have to put the trained model, followed by this:  .feature_importances_
        (as this example, were we have a trained random forest model, called "rf":  rf.feature_importances_ )
    names = here we put the name of the columns we want to show the feature importances of
